...
{
    public partial class FrmPersonelMusteriL : Form
    {
        public FrmPersonelMusteriL()
        {
            InitializeComponent();
        }
        ...
        {
            baglan.Open();
            SqlDataAdapter komut = new SqlDataAdapter("Select * From TblMusteri where MDurum in (1)", baglan);
            DataTable dt = new DataTable();
            komut.Fill(dt);
            gridControl1.DataSource = dt;
            baglan.Close();
        }
        private void FrmPersonelMusteriL_Load(object sender, EventArgs e)
        {
            listele();
